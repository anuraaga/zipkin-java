...
final class ElasticsearchSpanConsumer implements SpanConsumer {
  ...
  ElasticsearchSpanConsumer(Client client, String index, IndexNameFormatter indexNameFormatter,
      Clock clock) {
    this.client = client;
    this.index = index;
  ...
  private IndexRequestBuilder createSpanIndexRequest(Span span) {
    ...
    if (span.timestamp != null) {
      indexTimestamp = Instant.ofEpochMilli(TimeUnit.MICROSECONDS.toMillis(span.timestamp));
    } else {
      indexTimestamp = clock.instant();
    }
    String spanIndex = indexNameFormatter.formatIndexName(index, indexTimestamp);
